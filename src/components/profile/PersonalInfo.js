...
function PersonalInfo() {
  ...
  const validateDOB = (date) => {
    ...
    const diff = currentDate.getFullYear() - inputDate.getFullYear();
    return diff >= 10;
  };
  ...
  const save_changes = () => {
    if (!firstname || !lastname || !email || !phone || !dob || !country) {
    ...
    if (!validateDOB(dob)) {
      toast.error("Date of birth must be at least 10 years ago.");
      return;
    }
    const formdata = {
      image: newImage,
